...
public class View extends JFrame implements Observer {
	...
	public View(Model model) {
		...
		/*
		 * fieldSize = model.getPlayerShips().getGameFieldSize(); playerButtons
		 * = new FieldButton[fieldSize][fieldSize]; enemyButtons = new
		 * FieldButton[fieldSize][fieldSize]; // leftButtonArray = new
		 * JButton[fieldSize][fieldSize]; // leftButtonArray = new
		 * JButton[fieldSize][fieldSize];
		 * 
		 * gameFields.add(playerGameField); gameFields.add(enemyGameField);
		 * 
		 * buttonFields.add(playerButtons); buttonFields.add(enemyButtons);
		 * 
		 * // panels.add(leftPanel); // panels.add(rightPanel);
		 * 
		 * for (FieldButton[][] buttonField: buttonFields){ for (int x=0; x <
		 * fieldSize; x++){ for (int y=0; y < fieldSize; y++){ buttonField[x][y]
		 * = new FieldButton(x,y); buttonField[x][y].setOpaque(true);
		 ...
		 * 
		 * //gameField.setBackground(Color.gray); gameField.setPreferredSize(new
		 * Dimension(450,450)); } /* for (JPanel panel: panels){
		 ...
		 * 
		 * 
		 * //rightPanel.setBackground(Color.gray); middlePanel.add(leftPanel);
		 * middlePanel.add(rightPanel); //middlePanel.add(button);
		 * mainPanel.add(middlePanel, BorderLayout.CENTER);
		 * 
		 * mainPanel.setBorder(new EmptyBorder(10, 10, 10, 10)); add(mainPanel);
		 */
	...
	public void update(Observable arg0, Object arg1) {
		...
		// updateButtonField(playerPanel.getButtonField(),
		// model.getPlayerShips().getGameField());
		// updateButtonField(enemyPanel.getButtonField(),
		// model.getEnemyShips().getGameField());
	...
	/*
	 * public void updateButtonField(FieldButton[][] buttonField, GameField
	 * gameField){ for (int x=0; x < fieldSize; x++){ for (int y=0; y <
	 * fieldSize; y++){ Field field = gameField.getFieldAt(x, y); String content
	 * = field.getContent(); String labelText = "";
	 * 
	 * if (content != "water") buttonField[x][y].setBackground(Color.gray); if
	 * (content == "water") labelText = ""; if (content.contains("Battleship"))
	 * labelText = "B"; if (content.contains("Destroyer")) labelText = "D"; if
	 * (content.contains("Cruiser")) labelText = "C"; if
	 * (content.contains("Submarine")) labelText = "S"; if (content == "water"
	 * && field.isHit()) buttonField[x][y].setBackground(Color.cyan); if
	 * (content != "water" && field.isHit())

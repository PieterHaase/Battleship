...
public class AI {
	...
	public void makeTurn(){
		...
		if (field.getContent() != "water" && field.getParent().isSunk()){
			previousHits.clear();
			System.out.println("Computer sunk " + field.getParent().getType() + " '" + field.getParent().getName() + "'");
		}
	}
	...
	private Field pickRandomNeighbor(){		//wählt einen zufälligen Nachbarn des zuletzt gewählten Feldes
	...
	private Field pickNextFieldInLine(){		//wählt das nächste Feld, welches in einer Reihe mit den zwei zuletzt gewählten Feldern liegt und ein direkter Nachbar ist
		...
		int x = first.getXPos() - second.getXPos();						//hier muss noch das "umdrehen" implementiert werden
